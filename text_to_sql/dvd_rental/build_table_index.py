...
def build_embeddings(model: object, tables: dict[str, str]) -> dict:
    """Посчитать embedding-вектор для описания каждой таблицы"""

    result = {}

    names = list(tables.keys())
    descriptions = list(tables.values()) 
...
if __name__ == "__main__":
    ...
    table_descriptions = read_tables_from_json(f"metadata/tables_{VERSION}.json")
    pprint(table_descriptions)
    #print(f"{type(table_descriptions)=}")
    #print(f"{len(table_descriptions)=}")
    index = build_embeddings(model_sentence, table_descriptions)
    save_to_pickle(f"index/table_embeddings_{VERSION}.pkl", index)
    #obj = read_from_pickle(f"index/table_embeddings_{VERSION}.pkl")
    #pprint(obj)
